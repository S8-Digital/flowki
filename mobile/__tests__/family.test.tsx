...
vi.mock('react-native-paper', async () => {
    ...
    const stub = ({ children }: { children?: React.ReactNode }) => React.createElement('div', {}, children ?? null);
    ...
    const Button = ({
        ...
        testID?: string;
        mode?: string;
    }) => React.createElement('button', { onClick: onPress, disabled: disabled || loading, 'data-testid': testID, 'data-mode': mode }, children);
    ...
    const Avatar = {
        Text: ({ label }: { label?: string; size?: number }) => React.createElement('div', { 'data-testid': 'avatar', 'aria-label': label }, label),
    };

    const Chip = ({ children, onPress }: { children?: React.ReactNode; onPress?: () => void; compact?: boolean; icon?: string }) =>
    ...
    const Dialog = Object.assign(
        ...
        {
            Title: ({ children }: { children?: React.ReactNode }) => React.createElement('h2', { 'data-testid': 'dialog-title' }, children),
            Content: stub,
            Actions: stub,
        },
    );
...
vi.mock('@/components/ThemedText', () => ({
    ThemedText: ({ children }: { children?: React.ReactNode }) => React.createElement('span', {}, children ?? null),
}));
...
describe('Family screen', () => {
    ...
    describe('when the user has no family', () => {
        ...
        it('shows "Create a Family" button', async () => {
            render(React.createElement(FamilyScreen));
            await waitFor(() => expect(screen.getByRole('button', { name: /Create a Family/i })).toBeInTheDocument());
        });

        it('shows "Join with Invite Code" button', async () => {
            render(React.createElement(FamilyScreen));
            await waitFor(() => expect(screen.getByRole('button', { name: /Join with Invite Code/i })).toBeInTheDocument());
        });
        ...
        it('opens the Join dialog when "Join with Invite Code" is clicked', async () => {
            render(React.createElement(FamilyScreen));
            await waitFor(() => screen.getByRole('button', { name: /Join with Invite Code/i }));
        ...
        it('calls familyApi.create and refreshUser on successful family creation', async () => {
            ...
            const createBtn = screen.getAllByRole('button', { name: /Create/i }).find((b) => b.textContent === 'Create');
            fireEvent.click(createBtn!);

            await waitFor(() => expect(mockFamilyApi.create).toHaveBeenCalledWith('My Family'));
        ...
        it('calls familyApi.join and refreshUser on successful join', async () => {
            ...
            const joinBtn = screen.getAllByRole('button', { name: /Join/i }).find((b) => b.textContent === 'Join');
            fireEvent.click(joinBtn!);

            await waitFor(() => expect(mockFamilyApi.join).toHaveBeenCalledWith('ABCD1234'));
            await waitFor(() => expect(mockRefreshUser).toHaveBeenCalled());
        });
    });
    ...
    describe('when the user has a family', () => {
        ...
        it('shows member count', async () => {
            render(React.createElement(FamilyScreen));
            await waitFor(() => expect(screen.getByText(/Members \(2\)/i)).toBeInTheDocument());
        });
        ...
        it('copies the invite code to the clipboard when the copy button is pressed', async () => {
            ...
            await waitFor(() => expect(vi.mocked(Clipboard.setStringAsync)).toHaveBeenCalledWith('ABCD1234'));
        });
        ...
        it('shows location name when provided', async () => {
            ...
            render(React.createElement(FamilyScreen));
            await waitFor(() => expect(screen.getByText(/London, UK/i)).toBeInTheDocument());
        });
    });
});
