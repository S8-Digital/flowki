...
export default function Categories({ todoCategories: initialTodo, recipeCategories: initialRecipe, shoppingCategories: initialShopping }: Props) {
    ...
    function save() {
        setSaving(true);
        router.post('/settings/categories', { todo_categories: todoCategories, recipe_categories: recipeCategories, shopping_categories: shoppingCategories }, {
            onSuccess: () => { setSaved(true); setTimeout(() => setSaved(false), 2000); },
            onFinish: () => setSaving(false),
        });
    }

    function CategorySection({ title, description, categories, setter }: { title: string; description: string; categories: Category[]; setter: React.Dispatch<React.SetStateAction<Category[]>> }) {
        return (
                ...
                <div className="space-y-2">
                    {categories.map((cat, i) => (
                        <div key={i} className="flex items-center gap-2">
                            <Input value={cat.label} onChange={(e) => onLabelInput(setter, i, e.target.value, cat.value)} placeholder="Label (e.g. Household)" className="flex-1" />
                            <Input value={cat.value} onChange={(e) => onValueChange(setter, i, e.target.value)} placeholder="Value (e.g. household)" className="w-40 font-mono text-xs" />
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbItems}>
            ...
            <SettingsLayout>
                <CategorySection title="Todo Categories" description="Customize the categories available for todos." categories={todoCategories} setter={setTodoCategories} />
                <CategorySection title="Recipe Categories" description="Customize the categories available for recipes." categories={recipeCategories} setter={setRecipeCategories} />
                <CategorySection title="Shopping Categories" description="Customize the categories available for shopping items." categories={shoppingCategories} setter={setShoppingCategories} />
                <div className="flex items-center gap-4">
                    <Button onClick={save} disabled={saving}>{saving ? 'Saving…' : 'Save Categories'}</Button>
