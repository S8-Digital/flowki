...
export default function Profile({ mustVerifyEmail, status, hasGoogleCalendarConnected }: Props) {
    const page = usePage<{ auth: { user: { id: number; name: string; email: string; email_verified_at: string | null }; connectedProviders: string[]; hasPasswordSet: boolean } }>();
    ...
    function connectGoogleCalendar() { router.visit('/auth/google/calendar'); }
    ...
    function linkProvider(providerKey: string) { router.visit(link(providerKey).url); }

    function unlinkProvider(providerKey: string) {
        const remaining = connectedProviders.length;
        if (remaining <= 1 && !hasPasswordSet) { alert('You cannot unlink your only sign-in method. Please set a password first.'); return; }
            ...
            <SettingsLayout>
                        ...
                        {mustVerifyEmail && !user.email_verified_at && (
                            <div>
                                ...
                                {status === 'verification-link-sent' && (
                                    <p className="mt-2 text-sm font-medium text-green-600">A new verification link has been sent to your email address.</p>
                                )}
                            </div>
                        )}
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        {connectedProviders.includes(provider.key) ? (
                                            <p className="flex items-center gap-1 text-xs text-green-600 dark:text-green-400"><CheckCircle2 className="size-3" /> Connected</p>
                                        ) : (
                ...
                <div className="flex flex-col space-y-6">
                    ...
                    <div className="rounded-lg border p-4">
                        ...
                        {status === 'google-calendar-connected' && <div className="mt-3 text-sm font-medium text-green-600">Google Calendar connected successfully!</div>}
                        {status === 'google-calendar-disconnected' && <div className="mt-3 text-sm text-muted-foreground">Google Calendar has been disconnected.</div>}
                    </div>
                </div>
