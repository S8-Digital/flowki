...
<template>
    <div class="flex flex-col gap-2">
        <div v-if="todos.length === 0" class="py-8 text-center text-sm text-muted-foreground">No todos due today.</div>
        <ul v-else class="space-y-2">
            <li v-for="todo in todos" :key="todo.id" class="flex items-center gap-3 rounded-lg border p-3">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs text-muted-foreground capitalize">{{ todo.category }}</p>
                </div>
